                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <br><small class="text-muted">{{ ucfirst($application->buyer_type) }} {{ ucfirst(str_replace('_', ' ', $application->buyer_category)) }}</small>
                        </td>
                        <td>
                            <strong>{{ $application->sellApplication->seller->name }}</strong>
                            <br><small class="text-muted">Available: {{ number_format($application->sellApplication->share_quantity_to_sell) }} shares</small>
                        </td>
                        <td>{{ number_format($application->share_quantity_to_buy) }}</td>
                        <td>Rs. {{ number_format($application->offered_price_per_share, 2) }}</td>
                        <td>Rs. {{ number_format($application->share_quantity_to_buy * $application->offered_price_per_share, 2) }}</td>
                        <td>
                            <span class="badge bg-{{ 
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                <a href="{{ route('buy-applications.show', $application->id) }}" class="btn btn-sm btn-outline-info">
                                ...
                                <form action="{{ route('buy-applications.update-status', $application->id) }}" method="POST" class="d-inline">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-outline-success" onclick="return confirm('Approve this application?')">
                                ...
                                <form action="{{ route('buy-applications.update-status', $application->id) }}" method="POST" class="d-inline">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Reject this application?')">
