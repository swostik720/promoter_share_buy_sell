                    ...
                    <form action="{{ route('board-decisions.update', $decision->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="border p-3 bg-light rounded">
                                ...
                                <br><small class="text-muted">Applied:
                                    {{ $decision->sellApplication->application_date->format('M d, Y') }}</small>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="decision_date" class="form-label">Decision Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" class="form-control @error('decision_date') is-invalid @enderror"
                                    id="decision_date" name="decision_date"
                                    value="{{ old('decision_date', $decision->decision_date->format('Y-m-d')) }}" required>
                                @error('decision_date')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('decision') is-invalid @enderror" id="decision"
                                    name="decision" required>
                                    <option value="">Select Decision</option>
                                    <option value="approved"
                                        {{ old('decision', $decision->decision) == 'approved' ? 'selected' : '' }}>Approved
                                    </option>
                                    <option value="rejected"
                                        {{ old('decision', $decision->decision) == 'rejected' ? 'selected' : '' }}>Rejected
                                    </option>
                                </select>
                                @error('decision')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                class="form-control @error('meeting_minute_reference') is-invalid @enderror"
