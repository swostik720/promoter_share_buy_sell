                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @forelse($documents as $document)
                                <tr>
                                    <td>
                                        <span class="badge bg-info">{{ ucwords(str_replace('_', ' ', $document->document_type)) }}</span>
                                    </td>
                                    <td>{{ $document->file_name }}</td>
                                    <td>
                                        @if($document->documentable_type === 'App\Models\SellApplication')
                                            <span class="text-primary">Sell Application #{{ $document->documentable_id }}</span>
                                        @elseif($document->documentable_type === 'App\Models\BuyApplication')
                                            <span class="text-success">Buy Application #{{ $document->documentable_id }}</span>
                                        @else
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('documents.view', $document->id) }}" class="btn btn-outline-primary" target="_blank">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('documents.download', $document->id) }}" class="btn btn-outline-success">
                                                <i class="fas fa-download"></i>
                                            </a>
                                            @if(!$document->is_verified)
                                                <button class="btn btn-outline-info" onclick="verifyDocument({{ $document->id }})">
